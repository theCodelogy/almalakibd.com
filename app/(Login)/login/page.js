...
const page = () => {
    ...
    const { createUseWithEmail, signIngWithGoogle, user ,handleUpdateProfile,signIn } = useContext(authContext)
...
const handleGoogle = () =>{
    ...
    return (
        <div>
             <div className="w-80 md:w-96 lg:w-full mx-auto bg-white flex items-center relative overflow-hidden shadow-xl">
                {/* register form  */}
                <form onSubmit={handleRegister} className={`p-8 w-full ${register ? 'lg:translate-x-0' : 'lg:-translate-x-full hidden lg:block'} duration-500`}>
                    ...
                    <div className="space-y-5">
                        ...
                        <input id="name" type="name" name='userName' placeholder="John Doe" className="p-3 block w-full outline-none border rounded-md invalid:border-red-700 valid:border-black" />
                        ...
                        <input id="u_password" type="u_password" name='password' placeholder=".............." min={5} className="p-3 block w-full outline-none border rounded-md invalid:border-red-700 valid:border-black"/>
                    </div>
                    {/* button type will be submit for handling form submission*/}
                    <button className="py-2 px-5 mb-4 mx-auto mt-8 shadow-lg border rounded-md border-black block">Register Now</button>

                    <p className="mb-3 text-center">Already have an account?<Link onClick={() => {setRegister(!register);}} href="#" className="underline font-semibold">Login</Link></p>
                    <hr />
                    <button onClick={handleGoogle} className="py-2 px-5 mb-4 mt-8 mx-auto block shadow-lg border rounded-md border-black"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className="w-5 h-5 fill-current inline-block mr-2"><path d="M16.318 13.714v5.484h9.078c-0.37 2.354-2.745 6.901-9.078 6.901-5.458 0-9.917-4.521-9.917-10.099s4.458-10.099 9.917-10.099c3.109 0 5.193 1.318 6.38 2.464l4.339-4.182c-2.786-2.599-6.396-4.182-10.719-4.182-8.844 0-16 7.151-16 16s7.156 16 16 16c9.234 0 15.365-6.49 15.365-15.635 0-1.052-0.115-1.854-0.255-2.651z"></path></svg>Continue with Google</button>
                </form>
                {/* img */}
                <div className={`hidden lg:block absolute w-1/2 h-full top-0 z-50 duration-500 overflow-hidden bg-black/20 ${register ? 'translate-x-full rounded-bl-full duration-500' : 'rounded-br-full'}`}>
                    <Image src={image} className="object-cover h-full" alt="card navigate ui" />
                </div>
                ...
                <form onSubmit={handleLogin} className={`p-8 w-full mr-0 ml-auto duration-500 ${register ? 'lg:translate-x-full hidden lg:block' : ''}`}>
                    ...
                    <p className="mb-3 text-center">Don&apos;t have an account?<Link href="#" onClick={() => {setRegister(!register);}} className="underline font-semibold">Register</Link></p>
                    <hr />
